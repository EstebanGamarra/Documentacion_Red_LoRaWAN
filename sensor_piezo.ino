...
static const u1_t PROGMEM DEVEUI[8]={ 0x91, 0x94, 0x05, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 };
void os_getDevEui (u1_t* buf) { memcpy_P(buf, DEVEUI, 8);}

// This key should be in big endian format (or, since it is not really a
// number but a block of memory, endianness does not really apply). In
// practice, a key taken from ttnctl can be copied as-is.
static const u1_t PROGMEM APPKEY[16]={ 0x90, 0xDA, 0x0C, 0xEA, 0x8F, 0x5A, 0xBC, 0x39, 0xA8, 0x0C, 0x41, 0x4C, 0x1A, 0x63, 0x0A, 0x9D }
;
...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
        ...
        uint32_t piezo = analogRead(A0) * 100;
        ...
        byte payload[2];
        payload[0] = highByte(piezo);
        payload[1] = lowByte(piezo);
